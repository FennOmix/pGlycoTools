...
class IsotopeSimple:
    def __init__(self, config, max_isotope_len = 7):
        ...
        self.modlist = [self.config.mod_dict[mod] for mod in self.config.protein_var_mod if mod]
        self.modlist += [self.config.mod_dict[mod] for mod in self.config.protein_fix_mod if mod]
        
        self.replace_element = {}
        self.check_element()
        
    def check_element(self, replace_element = None):
        if replace_element is not None: self.replace_element = replace_element
    ...
    def _chem_tuples(self, chem):
        ...
        return [(elem if elem not in self.replace_element else self.replace_element[elem], int(n)) for elem, n in items]
        
    def _check_chem(self, obj_list):
        for obj in obj_list:
            vec = self._chem_tuples(obj.chemical)
            for elem, n in vec:
                if not elem in self.chem_list:
...
if __name__ == '__main__':
    ...
    yint, mono_idx = isocalc.get_distribution(peptide, modification, glycan_comp, glyco_names)
